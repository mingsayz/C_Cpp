...
## 문자열 입출력 함수
  ...
  * scanf() 함수는 공백을 만날 때까지 입력을 받지만, gets() 함수는 공백을 포함하여 한 줄을 입력받는다.
    ...
    > usage : gets_s(a,sizeof(a)); -> 버퍼의 크기를 철저히 지키기 위해 사용
    gets_s를 사용하는 경우 범위를 넘으면 그 즉시 런타임(Runtime)오류가 발생
...
## 함수 포인터
  ...
  * 함수 포인터를 이용하면 형태가 서로 같은 서로 다른 기능의 함수를 선택적으로 사용할 수 있다.
  ...
  > ex) void(* fp)() = function; (반환값, 매개변수가 없는 경우)
  > fp();
  > ex) int(* fp)(int,int) = function (반환값이 int, 매개변수 int,int를 받는경우)
  > fp(3,5);
...
## 구조체
  ...
  * 2) 캐릭터,몬스터,학생,좌표 등 다양한 객체를 모두 프로그래밍 언어를 이용해 표현할 수 있다.
    > usage: struct 구조체명 { 자료형1 변수명1 자료형2 변수명2 ....};
...
## 파일 입출력
  ...
  * 파일을 닫을 때는 fclose() 함수를 이용한다.
  > usage : FILE * fp; fp = fopen(파일 경로, 접근 방식); fclose(fp);
...
## 파일 입출력 함수
  ...
  * 파일 입출력에서는 그 대신에 fprintf(), fscanf()가 사용된다.
  > usage : fprintf(파일 포인터,서식,형식지정자); fscanf(파일 포인터,서식,형식지정자);
...
## 매크로 전처리기
  ...
  * #define 문법을 사용해 정의할 수 있다.
    > usage : #define PI 3.1415926535

  * define 문법에는 인자를 포함 할 수 있다.
    > usage : #define POW(x) (x * x)

  * #define 문법은 소스코드의 양을 획기적으로 줄여준다. (실무 혹은 알고리즘에서 자주 사용됨)
    > ex ) #define ll long long , #define ld long double
    > usage : ll a = 987654000; ld b = 100.5054;
---
